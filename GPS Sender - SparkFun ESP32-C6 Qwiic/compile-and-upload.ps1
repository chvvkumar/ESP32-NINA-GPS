...
if ($ARDUINO_CLI) {
    ...
    if ($SkipUpload) {
        Write-Host "`n[5/5] Skipping upload (SkipUpload flag set)" -ForegroundColor Yellow
        ...
        if (Test-Path $BIN_FILE) {
            ...
            if ($targetPath -and $targetPath -ne "") {
                ...
                $destFile = Join-Path $targetPath "ESP32-GPS.bin"
            } else {
                # Default to Downloads folder
                $downloadsPath = [System.IO.Path]::Combine($env:USERPROFILE, "Downloads")
                $destFile = Join-Path $downloadsPath "ESP32-GPS.bin"
            }
            ...
            Write-Host "      - Web OTA (if enabled in sketch)" -ForegroundColor Magenta
        } else {
    ...
    $uploadOutput = & $ARDUINO_CLI upload --fqbn $FQBN `
        ...
        $SKETCH_PATH 2>&1 | ForEach-Object {
            ...
            if ($line -match "Writing at 0x[0-9a-f]+.*?(\d+)\s*%") {
                $percent = $matches[1]
                Write-Host "`r      Upload progress: $percent%  " -NoNewline -ForegroundColor Cyan
            }
            # Show other important messages on new lines
            elseif ($line -match "Connecting|Connected to|Chip|Serial port|Uploading stub|Running stub|Changing baud|Configuring flash|Erasing flash|Compressed|Wrote \d+ bytes|Hash of data verified|Hard resetting") {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "  - ESP32 is connected to $ComPort" -ForegroundColor Yellow
    ...
    Write-Host "You can now open Serial Monitor to view output." -ForegroundColor Cyan
    exit 0
}
